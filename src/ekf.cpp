// respect karasawa localizer
#include "sq_scan_matcher/ekf.h"
EKF::EKF() : private_nh("~")
{
	private_nh.param("init_x", init_x, {0.0});
	private_nh.param("init_y", init_y, {0.0});
	private_nh.param("init_z", init_z, {0.0});
	private_nh.param("init_roll", init_roll, {0.0});
	private_nh.param("init_pitch", init_pitch, {0.0});
	private_nh.param("init_yaw", init_yaw, {0.0});
	private_nh.param("init_sigma", init_sigma, {1e-3});
	private_nh.param("sigma_imu", sigma_imu, {1e-3});
	private_nh.param("sigma_odom", sigma_odom, {1e-3});
	private_nh.param("sigma_match", sigma_match, {1e-3});
	private_nh.param("motion_noise_nn", motion_noise_nn, {1e-3});
	private_nh.param("motion_noise_no", motion_noise_no, {1e-3});
	private_nh.param("motion_noise_on", motion_noise_on, {1e-3});
	private_nh.param("motion_noise_oo", motion_noise_oo, {1e-3});
	private_nh.param("odom_flag", odom_flag, {true});
	private_nh.param("state_size", state_size, {6});

	/*subscriber*/
	sub_match_pose = nh.subscribe("/scan_matching/pose",10, &EKF::match_callback, this);
	sub_imu = nh.subscribe("/imu/data",10, &EKF::imu_callback, this);
	sub_odom = nh.subscribe("/odom",10, &EKF::odom_callback, this);
	/*publisher*/
	pub_ekf_pose = nh.advertise<geometry_msgs::PoseStamped>("/ekf/pose", 20); // lclへ送る？
	is_3DoF_ = false;	
...
void EKF::odom_callback(const nav_msgs::Odometry::ConstPtr& msg_odom)
{
...
void EKF::imu_callback(const sensor_msgs::ImuConstPtr& msg_imu)
{
	imu = *msg_imu;
	has_received_imu = true;
}
void EKF::match_callback(const geometry_msgs::PoseStamped::ConstPtr& msg_pose)
{
	ros::Time t_start = ros::Time::now();
	icp_pose = *msg_pose;
	has_received_icp_pose = true;
...
{
	if(is_3DoF_) state_size = 3;
	else state_size = 6;

	X_.setZero(state_size);
	P_.setZero(state_size, state_size);
	P_ = init_sigma * Eigen::MatrixXd::Identity(state_size, state_size);
	set_pose(x, y, z, roll, pitch, yaw);
}
void EKF::set_pose(double x, double y, double z, double roll, double pitch, double yaw)
{
	if(is_3DoF_){
		if(X_.size() != state_size){
...
{
	double nu = current_odom.twist.twist.linear.x;
	...
	M.setZero();
	M(0,0) = sigma_odom * sigma_odom;
	M(1,1) = sigma_imu * sigma_imu;
	...
	// A: 出力が指令値からずれた量に比例して遷移後の姿勢をどれだけずらすかを決める行列
...
void EKF::motion_update_6DoF(double dt)
{
	...
	double delta_roll = imu.angular_velocity.x * dt;
	double delta_pitch = imu.angular_velocity.y * dt;

	if(delta_yaw < 1e-3) delta_yaw = 0.0;
	if(delta_roll < 1e-3) delta_roll = 0.0;
	if(delta_pitch < 1e-3) delta_pitch = 0.0;

	Eigen::Vector3d delta_position = { current_odom.twist.twist.linear.x * dt, 0.0, 0.0 };
    ...
    std::cout << "rotation_rpy:" << std::endl;
    std::cout << rotation_rpy << std::endl;
...
void EKF::measurement_update_3DoF()
{
	...
	Z.setZero();
	Z(0) = icp_pose.pose.position.x;
	Z(1) = icp_pose.pose.position.y;
	Z(2) = calc_yaw_from_quat(icp_pose.pose.orientation);
	...
	R = sigma_match * Eigen::MatrixXd::Identity(X_.size(), X_.size());
	...
	X_ += 1.0 * K * Y; // カルマンゲインで調整
	P_ = (I - K*H) * P_; // 観測更新で更新んされた信念の共分散行列
}
...
void EKF::measurement_update_6DoF()
{
	...
	Z(0) = icp_pose.pose.position.x;
	Z(1) = icp_pose.pose.position.y;
	Z(2) = icp_pose.pose.position.z;
	calc_rpy_from_quat(icp_pose.pose.orientation, Z(3), Z(4), Z(5));
	...
	// R 測定ノイズ 対角行列*simgma_match
	Eigen::MatrixXd R = sigma_match * Eigen::MatrixXd::Identity(X_.size(), X_.size());
...
{
  std::cout << "is_3DoF: " << is_3DoF_ << std::endl;
...
// void EKF::respawn()
// {
// 	ekf_pose.pose.position.x = respawn_pose_.pose.position.x;
// 	ekf_pose.pose.position.y = respawn_pose_.pose.position.y;
// 	ekf_pose.pose.position.z = ndt_pose_.pose.position.z;
// 	ekf_pose.pose.orientation = ndt_pose_.pose.orientation;
// 	ekf_pose.header.frame_id = map_frame_id_;
...
{
	// ekf_pose.header.frame_id = map_frame_id_;
	ekf_pose.header.frame_id = "base_link";
	ekf_pose.pose.position.x = X_(0);
	ekf_pose.pose.position.y = X_(1);
	if(is_3DoF_){
		ekf_pose.pose.position.z = icp_pose.pose.position.z;
		//ekf_pose_.pose.position.z = 0.0;
		ekf_pose.pose.orientation = rpy_to_msg(0.0, 0.0, X_(2));
		...
		ekf_pose.pose.position.z = X_(2);
		ekf_pose.pose.orientation = rpy_to_msg(X_(3), X_(4), X_(5));
...
// 			tf2::Transform transform(q,tf2::Vector3(X_(0),X_(1), icp_pose.pose.position.z));
...
void EKF::process()
{
	...
	ros::Rate rate(10);
	while(ros::ok()){
		if(has_received_imu && has_received_odom){
			now_time_ = ros::Time::now();
			if(is_first_){
				dt = 1.0/(double)10;
				is_first_ = false;
			...
			// 	is_respawn_ = false;
			// }
            std::cout << "-----------motion_update-------------" << std::endl;
            motion_update(1.0/(double)10);
            std::cout << "-----------measurement_update-------------" << std::endl;
            if(has_received_icp_pose) measurement_update();
            //if(has_received_ndt_pose_ && !is_measurement_.data) measurement_update();
            publish_ekf_pose();
            // publish_tf();
			has_received_icp_pose = false;
		}

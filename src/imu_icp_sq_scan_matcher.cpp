#include "sq_scan_matcher/imu_icp_sq_scan_matcher.h"
ImuIcpSqScanMather::ImuIcpSqScanMather() : private_nh("~")
{
	...
	/*subscriber*/
	sub_pc = nh.subscribe("/velodyne_points", 1, &ImuIcpSqScanMather::cloud_callback, this);
	// sub_pose = nh.subscribe("/ekf/pose", 1, &ImuIcpSqScanMather::pose_callback, this);
  /*publisher*/
...
void ImuIcpSqScanMather::pose_callback(const geometry_msgs::PoseStampedConstPtr& msg_pose)
{
	pose_ekf = *msg_pose;
	first_callback_pose = false;
}
void ImuIcpSqScanMather::cloud_callback(const sensor_msgs::PointCloud2::ConstPtr& msg_cloud)
{
...
void ImuIcpSqScanMather::initial_registration(void)
{
	/*transform*/
...
bool ImuIcpSqScanMather::transformation(void)
{
...
void ImuIcpSqScanMather::Downsampling(CloudXYZIPtr pc, double leafsize)
{
...
void ImuIcpSqScanMather::PassThroughFilter(CloudXYZIPtr pc_in, CloudXYZIPtr pc_out, std::vector<double> range)
{
...
void ImuIcpSqScanMather::publication(void)
{
	/*publish*/
	pose_ndt.header.stamp = pose_ekf.header.stamp;
	// pose_ndt.header.frame_id = pose_ekf.header.frame_id;
	pose_ndt.header.frame_id = "map";
	pub_pose.publish(pose_ndt);
}
Eigen::Quaternionf ImuIcpSqScanMather::quat_msg_to_eigen(geometry_msgs::Quaternion q_msg)
{
...
geometry_msgs::Quaternion ImuIcpSqScanMather::quat_eigen_to_msg(Eigen::Quaternionf q)
{
...
{
    ros::init(argc, argv, "imu_icp_sq_scan_matcher");
    ImuIcpSqScanMather imu_icp_sq_scan_matcher;
    ros::spin();
    return 0;
}